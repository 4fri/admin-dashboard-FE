...
export default {
  ...
  created() {
    ...
    const alertId = this.$route.params.id;
  ...
  methods: {
    ...
    async fetchAlert(id){
      try {
        const response = await api.get(`/configurations/alerts/${id}/show`, {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${this.token}`
          }
        ...
        if (response.data.success) {
          const alert = response.data.result;
          this.form.connection.connection = alert.db_connection;
          this.form.category.tableName = alert.table_name;
          this.form.days = JSON.parse(alert.day_remaining);
          this.form.status = alert.status_active;
          this.fetchCategory();
        }
    ...
    async submitEdit(){
      this.form.days = this.form.days.map(day => parseInt(day, 10));
      try {
        const response = await api.put(`/configurations/alerts/${this.$route.params.id}/update`, {
          db_connection: this.form.connection.connection,
          table_name: this.form.category.tableName,
          day_remaining: this.form.days,
          status_active: this.form.status
        ...
        if (response.data.success) {
          this.$router.push('/configuration-alert'); // Redirect ke halaman user list
          this.$toast.success("Alert successfully updated!"); // Tampilkan notifikasi sukses
        } else {
          // Tampilkan pesan error dari response API
          this.$toast.error(response.data.message || "Failed to update Alert."); // Jika message tidak ada, gunakan pesan default
        }
      } catch (error) {
        ...
        if (error.response && error.response.data && error.response.data.message) {
          ...
          this.$toast.error("An error occurred while updating the user."); // Pesan default jika tidak ada pesan error
        }
      }
    }
  },
